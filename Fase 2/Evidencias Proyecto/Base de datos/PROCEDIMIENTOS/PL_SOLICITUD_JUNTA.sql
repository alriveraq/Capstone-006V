...
BEGIN
    ...
    IF v_count = 0 THEN
        u_error_code := 'PL_SJ_04';
        u_mensaje := 'El usuario no existe.';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO v_count FROM JUNTA_DE_VECINOS WHERE id_junta = u_id_junta;
    IF v_count = 0 THEN
        u_error_code := 'PL_SJ_05';
        u_mensaje := 'La junta no existe.';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO v_count FROM INFO_JUNTA WHERE id_presidente  = u_id_presidente_junta ;
    IF v_count = 0 THEN
        u_error_code := 'PL_SJ_06';
        u_mensaje := 'El presidente de la junta no existe.';
        RETURN;
    END IF;
    ...
    EXCEPTION
        WHEN OTHERS THEN
            u_mensaje := 'Error al procesar la solicitud: ' || SQLERRM;
            ROLLBACK;
    END;
